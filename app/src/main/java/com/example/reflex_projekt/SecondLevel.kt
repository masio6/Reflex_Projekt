 ...
 * Use the [SecondLevel.newInstance] factory method to
...
class SecondLevel : Fragment() {
    ...
    private var listOfRecords = mutableListOf<Stats3x3>()
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_second_level, container, false)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        (activity?.let {
            val builder =
                AlertDialog.Builder(it).setMessage("LEVEL 2 - plansza 3x3\n" +
    ...
    private fun dataInit() {
        runBlocking(Dispatchers.IO) {
            listOfRecords = appDatabase.stats3x3Dao().getAll()
        }
    }

    @SuppressLint("SetTextI18n")
    fun readRecords() {
        val list = mutableListOf<Stats3x3>()

        var maxPointsResult = 0
        var minTimeResult: Long = 10*60*1000

        var recordObject = Stats3x3(50000L, 0)

        if(listOfRecords.isNotEmpty()) {
            listOfRecords.forEach {
                if(it.points3x3 >= maxPointsResult) {
                    maxPointsResult = it.points3x3
                    list.add(it)
                }
            }

            list.forEach {
                if(it.time3x3 < minTimeResult) {
                    minTimeResult = it.time3x3
                    recordObject = it
                }
            }

            recordField.text = "Twój rekord: \nPunktów: ${recordObject.points3x3}/50\nCzas: ${recordObject.time3x3?.let {
                timeFormatConvert(
                    it
                )
            }}"
    ...
    fun writeResultToRecords(timeInMilis: Long, points: Int) {
        GlobalScope.launch(Dispatchers.IO) {
            appDatabase.stats3x3Dao().insert(Stats3x3(timeInMilis, points))
        }

        listOfRecords.add(Stats3x3(timeInMilis, points))
    }
    ...
    companion object {
         ...
         * @return A new instance of fragment SecondLevel.
         */
